...
const CANONICAL_FRAMEWORKS_FALLBACK = [
  {
...
export async function getFrameworksAction() {
  ...
  try {
    ...
    console.warn("getFrameworksAction backend fetch error, falling back to statutory catalog:", err?.message);
  }
  // Graceful canonical fallback guarantees UI never fails to load frameworks
  return { success: true, data: CANONICAL_FRAMEWORKS_FALLBACK, isFallback: true };
}
