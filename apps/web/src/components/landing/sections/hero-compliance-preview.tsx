...
export function HeroCompliancePreview() {
  ...
  return (
    <div 
      ...
      style={tiltStyle}
      className={`landing-reveal ${isRevealed ? 'revealed' : ''} w-full max-w-4xl mx-auto mt-10 rounded-[10px] rc-glass-smoked-elevated border border-[rgba(199,204,210,0.14)] shadow-[0_32px_80px_rgba(9,11,15,0.85)] overflow-hidden text-left transition-all hover:border-[rgba(188,167,123,0.35)] relative group`}
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-[#0C0F14]/95 border-b border-[rgba(199,204,210,0.08)] relative z-10">
        <div className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-full bg-[rgba(199,204,210,0.22)] border border-[rgba(199,204,210,0.10)]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[rgba(199,204,210,0.22)] border border-[rgba(199,204,210,0.10)]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[rgba(199,204,210,0.22)] border border-[rgba(199,204,210,0.10)]" />
          <span className="ml-3 font-mono text-[11px] text-[#C7CCD2] flex items-center gap-1.5">
            <Globe className="w-3 h-3 text-[#BCA77B]" />
            <span>cloud-portal.io &bull; Statutory Audit Result</span>
          </span>
        </div>

        <div className="flex items-center gap-2.5">
          <span className="font-mono text-[10px] text-[#76937F] bg-[#76937F]/15 px-2.5 py-0.5 rounded-[4px] border border-[#76937F]/35 flex items-center gap-1.5 font-medium">
      ...
      <div className="p-5 md:p-6 bg-[#080A0E]/90 relative z-10 min-h-[220px] flex flex-col justify-between">
        {/* Tab 1: Overview */}
        {activeTab === 'overview' && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-3.5 rounded-lg bg-[#12161C] border border-[rgba(199,204,210,0.08)]">
                ...
                <div className="text-2xl font-bold font-mono text-[#76937F] mt-1">94 / 100</div>
                <span className="text-[11px] text-[#C7CCD2] mt-0.5 block">High statutory adherence</span>
              </div>
              <div className="p-3.5 rounded-lg bg-[#12161C] border border-[rgba(199,204,210,0.08)]">
                <span className="text-[10px] font-mono text-[#969DA6] uppercase tracking-wider block">Checkpoints Verified</span>
                <div className="text-2xl font-bold font-mono text-[#FAF9F5] mt-1">18 / 20</div>
                <span className="text-[11px] text-[#76937F] mt-0.5 block">18 passed &bull; 2 need attention</span>
              </div>
              <div className="p-3.5 rounded-lg bg-[#12161C] border border-[rgba(199,204,210,0.08)]">
                <span className="text-[10px] font-mono text-[#969DA6] uppercase tracking-wider block">Surveillance Status</span>
                <div className="text-2xl font-bold font-mono text-[#BCA77B] mt-1">Active</div>
                <span className="text-[11px] text-[#C7CCD2] mt-0.5 block">Continuous automated monitoring</span>
              </div>
            </div>

            <div className="p-3.5 rounded-lg bg-[#171C23] border border-[rgba(199,204,210,0.10)] flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
              ...
              <button
                onClick={() => setActiveTab('remediation')}
                className="text-xs font-mono font-medium text-[#BCA77B] hover:text-[#FAF9F5] flex items-center gap-1 cursor-pointer shrink-0"
              >
                <span>View Fix</span>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-[#FAF9F5]">Enforce HSTS (HTTP Strict Transport Security)</span>
              </div>

              <div className="flex items-center gap-1 bg-[#12161C] border border-[rgba(199,204,210,0.10)] p-0.5 rounded-md text-xs font-mono">
                <button
            ...
            <div className="rounded-lg bg-[#12161C] border border-[rgba(199,204,210,0.10)] overflow-hidden font-mono text-xs">
              <div className="flex items-center justify-between px-3 py-1.5 bg-[#0C0F14] border-b border-[rgba(199,204,210,0.08)] text-[11px] text-[#969DA6]">
                <span>Drop-in Fix: {activeCodeTab === 'nextjs' ? 'next.config.ts' : '/etc/nginx/conf.d/security.conf'}</span>
                <button
                  onClick={handleCopy}
              ...
              <pre className="p-3 text-[#F3F4F2] overflow-x-auto leading-relaxed text-[11px] bg-[#0C0F14]">
                <code>
                  {activeCodeTab === 'nextjs'
                    ? `// next.config.ts\nexport default {\n  headers: async () => [{\n    source: '/:path*',\n    headers: [{ key: 'Strict-Transport-Security', value: 'max-age=31536000; includeSubDomains; preload' }],\n  }],\n};`
                    : `add_header Strict-Transport-Security "max-age=31536000; includeSubDomains; preload" always;`}
        ...
        {activeTab === 'frameworks' && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              const Icon = f.icon;
              return (
                <div key={f.name} className="p-3.5 rounded-lg bg-[#12161C] border border-[rgba(199,204,210,0.08)]">
                  <Icon className={`w-4 h-4 ${f.color} mb-2`} />
                  <div className="text-xs font-bold text-[#FAF9F5]">{f.name}</div>
                  <div className={`text-[11px] font-mono mt-1 ${f.color}`}>{f.status}</div>
                </div>
              );
            })}
          </div>
        ...
        {/* Bottom CTA Footnote */}
        <div className="mt-4 pt-3 border-t border-[rgba(199,204,210,0.08)] flex items-center justify-between text-xs">
